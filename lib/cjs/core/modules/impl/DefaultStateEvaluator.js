...
class DefaultStateEvaluator {
    ...
    async eval(executionContext) {
        return this.doReadStateHistory(executionContext.sortedInteractions, new StateEvaluator_1.EvalStateResult(executionContext.contractDefinition.initState, {}, {}), executionContext).then((results) => results[results.length - 1]);
    }
    async evalHistory(executionContext) {
        return this.doReadStateHistory(executionContext.sortedInteractions, new StateEvaluator_1.EvalStateResult(executionContext.contractDefinition.initState, {}, {}), executionContext);
    }
    async doReadStateHistory(missingInteractions, baseState, executionContext) {
        var _a;
        ...
        const initalEvalStateResult = new StateEvaluator_1.EvalStateResult(currentState, validity, errorMessages);
        const intialSortKeyCacheResult = new SortKeyCache_1.SortKeyCacheResult(currentSortKey, initalEvalStateResult);
        const stateUpdateHistory = [intialSortKeyCacheResult];
        for (let i = 0; i < missingInteractionsLength; i++) {
            ...
            const errorMessagesLocal = {
                [missingInteraction.id]: errorMessages[missingInteraction.id]
            };
            const validityLocal = {
                [missingInteraction.id]: validity[missingInteraction.id]
            };
            const updateEvalStateResult = new StateEvaluator_1.EvalStateResult(currentState, validityLocal, errorMessagesLocal);
            const updateSortKeyCacheResult = new SortKeyCache_1.SortKeyCacheResult(currentSortKey, updateEvalStateResult);
